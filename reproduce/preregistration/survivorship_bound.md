# Locked plan: bounding the survivorship bias on the Bybit and Binance panels
...
The shipped depth-10 parquet behind the Bybit panel carries `ts`, while the engine's
order-book parser was later changed to prefer `cts` where the archive has it, which is why a
fresh parse of a post-2024-01 coin-day no longer reproduces the panel. Measuring delisted
coins on `cts` would place a book-time shift of 3 ms median on BTC and 16 ms median on a
thinner symbol inside a dead-minus-live contrast, so the panel's basis is used and the
`ts_semantics` field is recorded on every row.
